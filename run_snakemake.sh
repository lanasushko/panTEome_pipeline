...
snakemake --latency-wait 20 --rerun-incomplete --cluster "qsub -cwd -w e -N {cluster.job_name} -l h_rt={cluster.time} -l h_vmem={cluster.mem} -pe {cluster.pe} {cluster.threads} -o {cluster.o} -e {cluster.e} -j {cluster.join} -M {cluster.email}" --cluster-config cluster_config.yml --jobs 100
